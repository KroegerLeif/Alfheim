...
mkdir -p "${INSTALL_DIR}"
mkdir -p "${INSTALL_DIR}/infrastructure/caddy"
mkdir -p "${INSTALL_DIR}/infrastructure/keycloak/providers"
...
fetch_asset "infrastructure/caddy/Caddyfile" "${INSTALL_DIR}/infrastructure/caddy/Caddyfile"
fetch_asset "infrastructure/keycloak/alfheim-realm.json" "${INSTALL_DIR}/infrastructure/keycloak/alfheim-realm.json"
fetch_asset "infrastructure/keycloak/providers/alfheim-theme.jar" "${INSTALL_DIR}/infrastructure/keycloak/providers/alfheim-theme.jar"
...
if [[ "${START_STACK}" == "true" ]]; then
  ...
  echo -e "  ├── ${CYAN}infrastructure/caddy/Caddyfile${RESET}                 # Central ingress reverse-proxy configuration"
  echo -e "  ├── ${CYAN}infrastructure/keycloak/alfheim-realm.json${RESET}     # OIDC realm definition"
  echo -e "  ├── ${CYAN}infrastructure/keycloak/providers/*.jar${RESET}        # Compiled Keycloak theme"
