...
func TestLoad_Defaults(t *testing.T) {
	...
	if cfg.InternalToken != "" {
		t.Errorf("expected empty InternalToken, got %q", cfg.InternalToken)
	}
}
...
func TestLoad_CustomEnv(t *testing.T) {
	...
	t.Setenv("ALFHEIM_INTERNAL_TOKEN", "  s3cret  ")
	...
	if cfg.InternalToken != "s3cret" {
		t.Errorf("expected trimmed InternalToken s3cret, got %q", cfg.InternalToken)
	}
}
