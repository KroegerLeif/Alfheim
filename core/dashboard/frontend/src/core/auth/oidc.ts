...
const VERIFIER_KEY = 'alfheim_oidc_pkce_verifier';
const STATE_KEY = 'alfheim_oidc_state';
const RETURN_TO_KEY = 'alfheim_oidc_return_to';
const TOKEN_KEY = 'alfheim_oidc_tokens';
...
/**
 * Resolves the browser-facing OIDC issuer URL.
 *
 * Resolution order:
 *  1. `window.__ALFHEIM_ENV__.OIDC_ISSUER` (runtime SSR injection)
 *  2. `NEXT_PUBLIC_OIDC_ISSUER` build-time variable
 *  3. `${window.location.origin}` as a last-resort fallback
 */
...
export function loadOidcConfig(): OidcConfig {
  ...
  return {
    ...
    clientId: process.env.NEXT_PUBLIC_OIDC_CLIENT_ID || 'dashboard-frontend',
    redirectUri,
