...
async def test_get_overdue_tasks_and_update(db_session_isolated: AsyncSession):
    ...
    # Test Overdue Tasks
    overdue = await TaskService.get_overdue_tasks(db_session, household_id=h1_id)
    ...
    updated_step = await TaskService.update_task_state(db_session, s1.id, update_payload, household_id=h1_id)
    assert updated_step.supply_needed_date == new_date
    assert updated_step.description == "Deferred"

    # Should no longer be overdue
    overdue_now = await TaskService.get_overdue_tasks(db_session, household_id=h1_id)
...
async def test_submit_maintenance_wizard_and_history(db_session_isolated: AsyncSession):
    ...
    with patch("app.features.tasks.service.TaskService.forward_supplies_to_shopping") as mock_forward:
        event = await TaskService.submit_maintenance_wizard(
            db_session, payload, household_id=h1_id, authorization="Bearer caller-token"
        )
        assert event.performer == "Alice"
        assert event.completed_steps is not None and "Clean Coil" in event.completed_steps
        mock_forward.assert_called_once_with(["Coil Cleaner"], h1_id, "Bearer caller-token")

    # Test Get History
    history = await TaskService.get_history(db_session, household_id=h1_id)
