...
async def test_get_households(db_session: AsyncSession, sample_household: Household):
    """Test retrieving the authenticated user's households."""
    assert sample_household.id is not None
    households = await DeviceService.get_households(db_session, household_id=sample_household.id)
    assert len(households) == 1
    assert households[0].id == sample_household.id
...
async def test_create_device_household_not_found(db_session: AsyncSession):
    ...
    with pytest.raises(HouseholdNotFoundError) as exc_info:
        await DeviceService.create_device(db_session, payload, household_id=999999)
...
async def test_get_device_by_id_not_found(db_session: AsyncSession):
    ...
    with pytest.raises(DeviceNotFoundError) as exc_info:
        await DeviceService.get_device_by_id(db_session, 888888, household_id=1)
