...
export function HistoryView() {
  ...
  // Fetch service history events from the dedicated backend endpoint.
  // The backend already sorts results date DESC and joins device_name / device_location.
  ...
  if (isError) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <p className="text-sm text-red-400">Failed to load service history. Please try again.</p>
      </div>
    );
  }
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-2">
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[10px] font-semibold text-slate-500 uppercase tracking-wider">
                      ...
                      <span>•</span>
                      {/* Device name and location come directly from the backend — no client-side join needed */}
                      <span className="text-cyan-400 font-bold">{event.device_name}</span>
                ...
                {event.completed_steps && event.completed_steps.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {event.completed_steps.map((step, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"
                      >
