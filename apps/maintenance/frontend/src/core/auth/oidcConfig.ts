...
export function loadOidcConfig(): OidcConfig {
  const redirectUri =
    ...
    (typeof window !== "undefined" ? `${window.location.origin}/maintenance/en` : "http://localhost:3000/maintenance/en");

  return {
    issuer: resolveOidcIssuer(),
    clientId: process.env.NEXT_PUBLIC_OIDC_CLIENT_ID || "maintenance-frontend",
    redirectUri,
