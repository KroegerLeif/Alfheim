...
import {
  useShoppingLists, useShoppingListDetails, useAddShoppingItem,
...
export default function ShoppingDashboard() {
  ...
  const { data: listsData, isLoading: listsLoading, isError: listsError, error: listsErrObj, refetch: refetchLists } = useShoppingLists();
  ...
  if (listsError) return <ShoppingErrorBanner listsErrObj={listsErrObj} refetchLists={refetchLists} />;
          ...
          {(!isMobile || mobileView === "add") && (
            <div className="flex flex-col gap-3.5 min-h-0">
              ...
              <div className="glass-inset rounded-xl p-3 flex items-center gap-3 shrink-0 select-none">
                <PantryBadge />
                <p className="font-sans text-[11px] text-muted-foreground/70 leading-tight">{t("pantryLegend")}</p>
              </div>
            </div>
          )}
        </div>
