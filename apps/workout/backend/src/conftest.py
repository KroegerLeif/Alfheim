...
from backend_shared.household import HouseholdRole, get_membership_lookup
...
from src.features.session.models import SessionExercise, SessionSet, WorkoutSession  # noqa: F401
...
@pytest.fixture
def member_headers(membership: StaticMembershipLookup) -> Callable[..., dict[str, str]]:
    """Factory registering ``sub`` as a member of ``household_id`` and returning its request headers."""

    def _member_headers(
        household_id: object, sub: str = TEST_USER_SUB, role: HouseholdRole = "MEMBER"
    ) -> dict[str, str]:
        membership.set(str(household_id), sub, role)
        return _auth_headers(sub=sub, household_id=household_id)

    return _member_headers
