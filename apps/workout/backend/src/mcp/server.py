...
@mcp.tool()
def get_workout_status() -> str:
    """Get the current status of the workout tracker backend."""
    return "Workout Tracker backend is running."
...
def discover_and_import_mcp_tools() -> None:
    ...
    for tools_path in features_dir.rglob("mcp_tools.py"):
        # Calculate module name relative to src, e.g., "src.features.equipment.mcp_tools"
