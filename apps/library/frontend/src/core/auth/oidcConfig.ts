...
export function loadOidcConfig(): OidcConfig {
  const redirectUri =
    ...
    (typeof window !== "undefined" ? `${window.location.origin}/library/en` : "http://localhost:3000/library/en");

  return {
    issuer: resolveOidcIssuer(),
    clientId: process.env.NEXT_PUBLIC_OIDC_CLIENT_ID || "library-frontend",
    redirectUri,
