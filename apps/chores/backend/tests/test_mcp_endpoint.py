...
async def mcp_client() -> AsyncIterator[AsyncClient]:
    ...
    with (
        ...
        async with app.router.lifespan_context(app):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://chores-backend:8000") as client:
                yield client
...
async def test_mcp_initialize_and_list_tools():
    ...
    assert {"get_chores_status", "assign_chore", "complete_chore_by_name"} <= set(tools)
