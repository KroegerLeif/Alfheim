# Workflow: App Architecture Review / Audit Checklist (`ai/workflow_app_audit.md`)

This workflow defines the mandatory step-by-step checklist to evaluate existing applications in the monorepo for architectural violations, potential crashes, and z-index overlap bugs.

---

## 🛠️ Step-by-Step Audit Workflow

Follow this checklist whenever evaluating an application's layout or before beginning a major refactoring sprint.

### 📋 Phase 1: File Size & Complexity Audit
- [ ] **Scan Page Components**: Locate all `page.tsx` and `layout.tsx` files inside `src/app/`.
- [ ] **Verify Line Limits**: Ensure no view, page, or layout component exceeds **200 lines of code**.
- [ ] **Identify Monoliths**: Flag any single component file housing local queries, modal layouts, list mappings, and forms simultaneously. Respect the Single Responsibility Principle.

### 📁 Phase 2: Feature-Driven Design (FDD) Isolation
- [ ] **Check Core Layers**: Ensure generic concerns (auth, query clients, shared providers, base HTTP clients) are organized cleanly inside `src/core/` or `src/shared/`.
...
- [ ] **Verify Parameter Promise Resolution**: If Next.js 15+, check that the page is an `async` Server Component resolving route params asynchronously (`const { id } = await params;`) instead of using client-side hooks (`useParams`).
- [ ] **Verify Suspense Boundaries**: Verify that the dynamic layout wraps client components inside a React `<Suspense>` fallback boundary to protect against hydration crashes during external redirects (e.g., OIDC auth code swaps).
...
- [ ] **Check Map Isolation**: Verify the map component is wrapped in a container with `relative z-0 isolate overflow-hidden`.
